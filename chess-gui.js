...
class ChessGUI {
    ...
    handleSquareClick(event) {
        ...
        if (this.game.getGameState() !== "UNFINISHED") {
            this.showMessage(`Game is finished! Winner: ${this.game.getGameState()}`, 'info');
            return;
        }
    ...
    selectSquare(row, col, chessNotation) {
        ...
        console.log(`Selecting square ${chessNotation}, piece: ${piece}, current player: ${this.game.getCurrentPlayer()}`);
        ...
        this.showMessage(`Selected ${this.game.getPieceSymbol(piece)} on ${chessNotation.toUpperCase()}`, 'info');
    }
    ...
    attemptMove(fromSquare, toSquare) {
        ...
        if (result.success) {
            this.clearSelection();
            this.updateDisplay();
            this.showMessage(`Move: ${fromSquare.toUpperCase()} → ${toSquare.toUpperCase()}`, 'success');
            
            // Check for game end
            if (this.game.getGameState() !== "UNFINISHED") {
                setTimeout(() => {
                    this.showMessage(`🎉 GAME OVER! Winner: ${this.game.getGameState()} 🎉`, 'success');
                }, 1000);
            }
        } else {
